...
class PartieTkinter:
    ...
    def __init__(self, button_frame, results_frame, graph_frame):
        ...
        self.canvas.get_tk_widget().grid(row=0, column=0, sticky='NSW')
    ...
    def _update_results(self):
        ...
        for idx, joueur in enumerate(self.partie.liste_joueurs):
            ...
            ttk.Label(self.results_frame,
                      text=joueur.identifiant, foreground=couleur
            ).grid(row=idx, column=0, sticky='W', padx=5, pady=2)
            ttk.Label(self.results_frame,
                      text=f"{joueur.portefeuille} $", foreground=couleur
            ).grid(row=idx, column=1, sticky='W', padx=5, pady=2)
            ttk.Label(self.results_frame,
                      text=joueur.strategie.nom_strategie, foreground=couleur
